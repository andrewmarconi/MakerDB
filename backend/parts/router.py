...
from parts.schemas import PartSchema, PartCreate, PartUpdate, TagsInput
...
def _get_part_queryset():
    return Part.objects.select_related('manufacturer', 'default_storage').prefetch_related('attachments')


@router.get("/", response_model=List[PartSchema])
async def list_parts():
    parts = await sync_to_async(list)(_get_part_queryset().all())
    return parts
